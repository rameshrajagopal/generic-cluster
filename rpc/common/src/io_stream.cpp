...
*/
void IOStream::send_response(int res_code, const header_map & h, std::shared_ptr<Buffer<uint8_t>> response)
{
    auto self = shared_from_this();
    ios.post([self, h, res_code, response]() {
            std::lock_guard<std::mutex> lg(self->mu);
            if (self->closed) {
                return;
            }
            self->res.write_head(res_code, h); 
            self->res.end([response](uint8_t * buf, size_t len, uint32_t * flags) -> ssize_t {
                size_t ret = response->copy(buf, len);
